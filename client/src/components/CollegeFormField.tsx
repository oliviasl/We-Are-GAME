...
export function CollegeFormFormField({form, field, isNested}: CollegeFormFieldProps) {
  if (field.type === "category") {
    return <div className={"grid grid-cols-[1fr_2fr] gap-8"}>
      ...
      <div className={`grid ${field.columns === 1 ? "grid-cols-1" : "grid-cols-2"} gap-x-8 gap-y-4`}>
        {field.fields.map((subfield) => <CollegeFormFormField key={subfield.id} form={form} field={subfield}
                                                              isNested/>)}
      </div>
    </div>
  }
  ...
  return <div className={`grid ${isNested ? "grid-cols-[1fr_1fr]" : "grid-cols-[1fr_2fr]"} gap-8`}>
    ...
    <div className={"space-y-1"}>
      <Input
        ...
        crossOrigin
      />
      {form.getFieldState(field.id).error &&
