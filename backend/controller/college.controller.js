...
class collegeController {
  ...
  sqlBuilder(fields, pageNumber) {
    ...
    if ("collegeByName" in fields && fields["collegeByName"] != "") {
      wheres.push(
        "LOWER(college_name) LIKE LOWER('%" + fields["collegeByName"] + "%')"
      );
    }
    ...
    if ("collegeBySATRead" in fields && fields["collegeBySATRead"] != "") {
      wheres.push(
        fields["collegeBySATRead"] +
          " BETWEEN min_sat_read_write AND max_sat_read_write"
      );
    }
    if ("collegeBySATMath" in fields && fields["collegeBySATMath"] != "") {
      wheres.push(
        fields["collegeBySATMath"] + " BETWEEN min_sat_math AND max_sat_math"
      );
    }
    ...
    if (
      "collegeHasStuAthAcademicRes" in fields &&
      fields["collegeHasStuAthAcademicRes"]
    ) {
      wheres.push("stu_ath_academic_res_web_addr IS NOT NULL");
    }
    if (
      "collegeHasAcademicResource" in fields &&
      fields["collegeHasAcademicResource"]
    ) {
      wheres.push("academic_resources_web_addr IS NOT NULL");
    }
    if (
      "collegeHasDiversityResource" in fields &&
      fields["collegeHasDiversityResource"]
    ) {
    ...
    const sqlStr =
      " ORDER BY college_name LIMIT " + PAGE_SIZE + " OFFSET " + offset + ";";

    if (wheres.length != 0) {
      return [
        query + " WHERE " + sqlWhere + ";",
        query + " WHERE " + sqlWhere + sqlStr,
      ];
    }
  ...
  async paginatedCollegesFiltered(fields, pageNumber) {
    const PAGE_SIZE = 7;
    const [filteredCollegeQuery, filteredPaginatedCollegeQuery] =
      this.sqlBuilder(fields, pageNumber);

    const filteredPaginatedCollegeResult = await db.query(
      filteredPaginatedCollegeQuery
    );
