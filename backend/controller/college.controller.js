...
class collegeController {
  ...
  constructor() {
    collegeController.filterMap.set("allColleges", () =>
      this.allColleges(false)
    );
    collegeController.filterMap.set("collegeByName", (college_name) =>
      this.collegeByName(college_name, false)
    );
    collegeController.filterMap.set("collegeById", (collegeId) =>
      this.collegeById(collegeId, false)
    );
    collegeController.filterMap.set("collegeHasStuAthAcademicRes", () =>
      this.collegeHasStuAthAcademicRes(false)
    );
    collegeController.filterMap.set("collegeHasAcademicResource", () =>
      this.collegeHasAcademicResource(false)
    );
    collegeController.filterMap.set("collegeHasDiversityResource", () =>
      this.collegeHasDiversityResource(false)
    );
    collegeController.filterMap.set("collegeByGPA", (gpa) =>
      this.collegeByGPA(gpa, false)
    );
    collegeController.filterMap.set("collegeBySATRead", (satReadWrite) =>
      this.collegeBySATRead(satReadWrite, false)
    );
    collegeController.filterMap.set("collegeBySATMath", (satMath) =>
      this.collegeBySATMath(satMath, false)
    );
    collegeController.filterMap.set("collegeByACT", (act) =>
      this.collegeByACT(act, false)
    );
  }
  ...
  async collegeById(collegeId, directCall = true) {
    ...
    if (directCall) {
      try {
        const result = await db.query(queryStr + ";", [collegeId]);
        return result.rows;
      } catch (error) {
        return error;
      }
  ...
  async collegeByGPA(gpa, directCall = true) {
    ...
    if (directCall) {
      const result = await db.query(queryStr + ";", [gpa]);
      return result.rows;
    } else {
      return [queryStr, gpa];
    }
  ...
  async collegeBySATRead(satReadWrite, directCall = true) {
    ...
    if (directCall) {
      const result = await db.query(queryStr + ";", [satReadWrite]);
      return result.rows;
    } else {
  ...
  async collegeBySATMath(satMath, directCall = true) {
    ...
    if (directCall) {
      const result = await db.query(queryStr + ";", [satMath]);
      return result.rows;
    } else {
  ...
  async collegeByACT(act, directCall = true) {
    ...
    if (directCall) {
      try {
        const result = await db.query(queryStr + ";", [act]);
        return result.rows;
      } catch (error) {
        return error;
      }
  ...
  async deleteCollege(collegeId) {
    ...
    await db.query(`DELETE FROM college_assignments WHERE college_id = $1`, [
      collegeId,
    ]);
  ...
  async createAssignment(userId, collegeId) {
    try {
      const checkQuery = `
            SELECT * FROM college_assignments WHERE user_id = $1 AND college_id = $2;`;
      const checkResult = await db.query(checkQuery, [userId, collegeId]);

      if (checkResult.rows.length > 0) {
        // returning already existing assignment
        ...
        const insertQuery = `
                ...
                `;
        const insertResult = await db.query(insertQuery, [userId, collegeId]);
  ...
  async deleteAssignment(userId, collegeId) {
    const result = await db.query(
      `DELETE FROM college_assignments WHERE user_id = $1 AND college_id = $2`,
      [userId, collegeId]
    );
    return result.rows;
  }
  ...
  async autofillCollege(name) {
    ...
    const DATA_TO_FETCH = [
      ...
      {
        columnName: "min_sat_read_write",
        objectPath:
          "latest.admissions.sat_scores.25th_percentile.critical_reading",
      },
      {
        columnName: "max_sat_read_write",
        objectPath:
          "latest.admissions.sat_scores.75th_percentile.critical_reading",
      },
      {
        columnName: "min_sat_math",
        objectPath: "latest.admissions.sat_scores.25th_percentile.math",
      },
      {
        columnName: "max_sat_math",
        objectPath: "latest.admissions.sat_scores.75th_percentile.math",
      },
      {
        columnName: "min_act",
        objectPath: "latest.admissions.act_scores.25th_percentile.cumulative",
      },
      {
        columnName: "max_act",
        objectPath: "latest.admissions.act_scores.75th_percentile.cumulative",
      },
      {
        columnName: "min_act_english",
        objectPath: "latest.admissions.act_scores.25th_percentile.english",
      },
      {
        columnName: "max_act_english",
        objectPath: "latest.admissions.act_scores.75th_percentile.english",
      },
      {
        columnName: "min_act_math",
        objectPath: "latest.admissions.act_scores.25th_percentile.math",
      },
      {
        columnName: "max_act_math",
        objectPath: "latest.admissions.act_scores.75th_percentile.math",
      },
      {
        columnName: "faculty_student_ratio",
        objectPath: "latest.student.demographics.student_faculty_ratio",
      },
      {
        columnName: "acceptance_rate",
        objectPath: "latest.admissions.admission_rate.overall",
        multiplier: 100,
      },
      {
        columnName: "race_white",
        objectPath: "latest.student.demographics.race_ethnicity.white",
        multiplier: 100,
      },
      {
        columnName: "race_black",
        objectPath: "latest.student.demographics.race_ethnicity.black",
        multiplier: 100,
      },
      {
        columnName: "race_hispanic",
        objectPath: "latest.student.demographics.race_ethnicity.hispanic",
        multiplier: 100,
      },
      {
        columnName: "race_asian",
        objectPath: "latest.student.demographics.race_ethnicity.asian",
        multiplier: 100,
      },
      {
        columnName: "race_native_american",
        objectPath: "latest.student.demographics.race_ethnicity.aian",
        multiplier: 100,
      },
      {
        columnName: "race_pacific_islander",
        objectPath: "latest.student.demographics.race_ethnicity.nhpi",
        multiplier: 100,
      },
      {
        columnName: "race_two_or_more",
        objectPath: "latest.student.demographics.race_ethnicity.two_or_more",
        multiplier: 100,
      },
      {
        columnName: "race_international",
        objectPath:
          "latest.student.demographics.race_ethnicity.non_resident_alien",
        multiplier: 100,
      },
      {
        columnName: "race_other",
        objectPath: "latest.student.demographics.race_ethnicity.unknown",
        multiplier: 100,
      },
      {
        columnName: "first_year_retention_rate",
        objectPath: "latest.student.retention_rate.four_year.full_time",
        multiplier: 100,
      },
      {
        columnName: "cost_tuition_in_state",
        objectPath: "latest.cost.tuition.in_state",
      },
      {
        columnName: "cost_tuition_out_of_state",
        objectPath: "latest.cost.tuition.out_of_state",
      },
      {
        columnName: "avg_total_cost_est",
        objectPath: "latest.cost.avg_net_price.overall",
      },
      {
        columnName: "net_price_calc_web_addr",
        objectPath: "latest.school.price_calculator_url",
      },
    ];
    ...
    DATA_TO_FETCH.forEach((datum) => {
      ...
      if (value) {
        ...
        if (datum?.multiplier) {
          collegeData[datum.columnName] = Math.round(
            collegeData[datum.columnName] * datum.multiplier
          ).toString();
        }
      }
    });
  ...
  async generateFilterQuery(fields) {
    ...
    for (const [idx, field] of filterFields.entries()) {
      if (fields[field] != null && fields[field] !== "") {
        ...
        if (res[0] !== undefined) {
          ...
          if (idx > 0 && queryStr.length > 0) {
            // Ensure INTERSECT is added only if needed
    ...
    queryStr.push(
      ` LIMIT $${queryParamLength + 1} OFFSET $${queryParamLength + 2}`
    );
  ...
  async paginatedCollegesFiltered(fields, pageNumber) {
    ...
    const partialResult = await this.collegesFiltered(
      fields,
      PAGE_SIZE,
      offset
    );
  ...
  async searchScorecard(name) {
    const response = await this.fetchFromScorecard(
      name,
      ["school.name"],
      false,
      0,
